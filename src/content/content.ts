...
export const projects: Project[] = [
  {
    ...
    title: "Sketch Lab",
    blurb:
      "Two CNNs I trained in PyTorch (MNIST digits + 75-class Quick, Draw! doodles), exported to ONNX and running live in the browser: play Pictionary against the doodle model on the ML Lab tab.",
    description:
      "The training side of this site's ML Lab. A shared conv trunk powers two models: a digit recognizer (99.4% MNIST test accuracy, reads multi-digit numbers via connected-component segmentation) and a 75-class doodle classifier trained on over a million Quick, Draw! sketches. Normalization and softmax are baked into the ONNX graphs, the browser preprocessing reproduces MNIST's own crop/scale/center construction, and a canvas-simulation test gates every export so mouse drawings actually classify well; the game's prompt list is generated from measured per-class robustness rather than hand-picked. Inference runs client-side via onnxruntime-web (WebAssembly): nothing you draw leaves the page.",

...
- ML Lab (/ml: Doodle Duel game + digit lab, in-browser ONNX) → `src/components/ml/*` + `src/app/ml/page.tsx`; model artifacts in `public/ml/` + `src/content/ml-metrics.json` (both generated by the sketch-lab training repo, github.com/kevinm126/sketch-lab); onnxruntime wasm synced to `public/ml/ort/` by `scripts/sync-ort-assets.mjs` (predev/prebuild)
